...
5. **Expand GDI coverage** — _expanded_
   - Added classic mapping/state exports for `SetRelAbs`, `OffsetWindowOrg`, `ScaleWindowExt`, `OffsetViewportOrg`, `ScaleViewportExt`, `GetPolyFillMode`, `GetRelAbs`, `GetTextCharacterExtra`, `GetTextFace`, `GetViewportExt`, `GetViewportOrg`, `GetWindowExt`, `GetWindowOrg`, `IntersectVisRect`, `OffsetVisRgn`, `SetDCOrg`, `GetBrushOrg`, `GetBitmapDimension`, `SetBitmapDimension`, and `GetAspectRatioFilter`.
   - Added additional classic drawing/bitmap exports for `SetPolyFillMode`, `SetTextCharacterExtra`, `SetTextJustification`, `Pie`, `Chord`, `CreateBitmapIndirect`, `SetBitmapBits`, `SetDIBits`, `GetDIBits`, `CreateDIBPatternBrush`, and `PolyPolygon`.
   ...
   - Added palette/font/color exports for `SelectPalette`, `RealizePalette`, `GetPaletteEntries`, `SetPaletteEntries`, `RealizeDefaultPalette`, `UpdateColors`, `AnimatePalette`, `ResizePalette`, `GetNearestPaletteIndex`, `SetSystemPaletteUse`, `GetSystemPaletteUse`, `AddFontResource`, and `DeviceColorMatch`.
