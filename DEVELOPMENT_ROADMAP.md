...
3. **Expand GDI coverage** — _expanded_
   - Added classic mapping/state exports for `OffsetWindowOrg`, `ScaleWindowExt`, `OffsetViewportOrg`, `ScaleViewportExt`, `GetPolyFillMode`, `GetTextCharacterExtra`, `GetViewportExt`, `GetViewportOrg`, `GetWindowExt`, `GetWindowOrg`, `GetBrushOrg`, `GetBitmapDimension`, and `SetBitmapDimension`.
